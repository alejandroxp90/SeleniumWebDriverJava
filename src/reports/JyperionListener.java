...
/*** JyperionListener ** @author janaudy at jyperion dot org*/
...
public class JyperionListener implements ITestListener {
	...
	public void onTestFailure(ITestResult result) {
		...
		/*****  Toma una ScreenShot cuando existe un error y adjunta un archivo PDF *****/
		String file = System.getProperty("user.dir") + "\\screenshot" + new Random().nextInt() + ".png";
		//Llamamos al metodo takeScreenShot del BaseClass
		try {
		...
		//cell = new PdfPCell(new Paragraph(exception));
		//this.failTable.addCell(cell);
		/***** código para adjuntar la captura de pantalla al informe PDF ******/
		Throwable throwable = result.getThrowable();
		if (throwable != null) {
			this.throwableMap.put(new Integer(throwable.hashCode()), throwable);
			this.nbExceptions++;
			...
			/***************************** Attach screenshots ****************************/
			//Creamos un objeto del tipo Chunk
			Chunk imdb = new Chunk("[SCREEN SHOT]", new Font(Font.TIMES_ROMAN, Font.DEFAULTSIZE, Font.UNDERLINE));
			//Damos attachment el archivo
			imdb.setAction(new PdfAction("file:///" + file ));//llamamos file para indicar que es el archivo.
			Paragraph excep = new Paragraph(throwable.toString());//Se agrega a un nuevo parrafo
			excep.add(imdb);//se indica cuales son los archivos que se deben tener como attachment
			cell = new PdfPCell(excep);//Lo agregamos a una nueva celda
			this.failTable.addCell(cell);//Se agrega a la tabla donde estan los errores
			/*************************************************************************** */
		} else {
